...
def test_decimal_decoder_default_separator():
    ...
    result, error = decoder.run("72 101 108 108 111")
...
def test_decimal_decoder_special_chars():
    decoder = DecimalDecoder()
    result, error = decoder.run("33 64 35 36")
    assert error is None
    assert result == "!@#$"

def test_decimal_decoder_invalid_input():
    decoder = DecimalDecoder()
    result, error = decoder.run("72 101 108 108 111 999")
    assert error is not None
    assert isinstance(error, ToolError)
    assert "not a valid extended ASCII code" in str(error)
    assert result == "72 101 108 108 111 999"

def test_decimal_decoder_domain_chars():
    decoder = DecimalDecoder()
    assert decoder.domain_chars == set("0123456789")
